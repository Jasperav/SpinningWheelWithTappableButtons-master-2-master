...
open class SpinWheelControl: UIControl {
    ...
    public func drawWheel() {
        ...
        for wedgeNumber in 0..<numberOfWedges {
            ...
            wedge.button.configureWedgeButton(index: wedgeNumber, width: radius * 0.45, position: spinWheelCenter, radiansPerWedge: radiansPerWedge, tag: Int(wedgeNumber))
    ...
    override open func endTracking(_ touch: UITouch?, with event: UIEvent?) {
        if !isSpinning{
            spinWheelView.point(inside: (touch?.location(in: self))!, with: event)
        }
        isSpinning = false
